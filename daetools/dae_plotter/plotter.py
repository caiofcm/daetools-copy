...
class daeMainWindow(QtWidgets.QMainWindow):
    def __init__(self, tcpipServer):
        ...
        newPlotsFromConfigFile = QtWidgets.QAction('New plot from the user configuration file..', self)
        newPlotsFromConfigFile.setShortcut('Ctrl+Y')
        newPlotsFromConfigFile.setStatusTip('New plot from the user configuration file')
        newPlotsFromConfigFile.triggered.connect(self.slotnewPlotsFromConfigFile)        
        ...
        plot.addAction(newPlotsFromConfigFile)
        plot.addSeparator()
...
#@QtCore.pyqtSlot()
    def slotnewPlotsFromConfigFile(self):
        try:
            filename, ok = QtWidgets.QFileDialog.getOpenFileName(self, "New plot from the user configuration file", "", "Config file (*.json)")
            if not ok:
                return
            ...
            curves         = config['new_plotter']

            plot2D = dae2DPlot(self, 0, True)
