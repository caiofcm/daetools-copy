...
from pyUnits import m, g, kg, s, K, Pa, mol, J, W, unit

dimless = unit()

dimless_t = daeVariableType(
    name="dimless_t", units=unit(), lowerBound=0.0,
    upperBound=1e20, initialGuess=1.0, absTolerance=1e-5)
...
class modTutorial(daeModel):
    def __init__(self, Name, Parent = None, Description = ""):
        ...
        self.rho     = daeVariable("rho", molar_concentration_t,            self, "rho")
        self.rho_m     = daeVariable("rho_m", density_t,            self, "rho mass")
        self.z     = daeVariable("z", no_t,            self, "z")

    def DeclareEquations(self):
        daeModel.DeclareEquations(self)

        # P, T, x arguments can be floats or adouble objects/arrays as illustrated here.
        R = Constant(8.314 * 1e-3 * J/(mol*K)) #to kmol ?
        P = 1e5 * Pa
        Pval = Constant(P)
        T = self.T()
        x = [0.60, 0.40]
        ...
        eq.Residual = self.rho() - self.tpp.rho(P,T,x) #P / (self.tpp.rho(P,T,x)*R*T)
        eq = self.CreateEquation("rho_m", "Density calculation")
        eq.Residual = self.rho_m() - self.rho() * self.tpp.M(P,T,x)

        # 4. Fake Compressibility Factor [-]
        eq = self.CreateEquation("z", "z")
        eq.Residual = self.z() - Pval / (self.rho()*R*self.T())
